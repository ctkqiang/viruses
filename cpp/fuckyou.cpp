...
//create some files 
{
	...
	if (f != INVALID_HANDLE_VALUE) {
		...
		WriteFile(f, "I just want to say this once, FUCK YOU !" , 105, &r, NULL);
		CloseHandle(f);
...
char txtfile[MAX_PATH]; 
	...
	if (f != INVALID_HANDLE_VALUE) {
		...
		WriteFile(f, "I WILL STAB YOU IN THE BACK OF THE HEAD.\r\n", 105, &r, NULL);
		CloseHandle(f);
...
char txtfile[MAX_PATH]; 
	...
	if (f != INVALID_HANDLE_VALUE) {
		...
		WriteFile(f, "I'M GOING TO MURDER YOU IN COLD BLOOD.  I'M FUCKING GOING TO SHOOT YOU WITH A M16 OVER YOUR GRAVE. \r\n", 105, &r, NULL);
		CloseHandle(f);
